...
public class FullDepthJSONTest {
  ...
  public void testUnmarshal() throws IOException {
    ...
    Assert.assertEquals(new Double("3260.40000"), anxFullDepth.getAsks().get(0).getPrice());
    Assert.assertEquals(326040000L, anxFullDepth.getAsks().get(0).getPriceInt());
    Assert.assertEquals(new Double("16.00000000"), anxFullDepth.getAsks().get(0).getAmount());
    Assert.assertEquals(1600000000L, anxFullDepth.getAsks().get(0).getAmountInt());

    Assert.assertEquals(4, anxFullDepth.getBids().size());
    Assert.assertEquals(new Double("2000.00000"), anxFullDepth.getBids().get(0).getPrice());
    Assert.assertEquals(200000000L, anxFullDepth.getBids().get(0).getPriceInt());
    Assert.assertEquals(new Double("3.00000000"), anxFullDepth.getBids().get(0).getAmount());
