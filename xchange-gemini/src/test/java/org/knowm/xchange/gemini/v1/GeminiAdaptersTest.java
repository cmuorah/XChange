...
public class GeminiAdaptersTest {
    ...
    public void testAdaptOrdersToOrdersContainer() {
        ...
        assertEquals(lastLevel.getTimestamp() * (new Double(1000L)).longValue(), container.getTimestamp(), 1e-8);
    ...
    public void testAdaptOrdersToOpenOrders() {
        ...
        for (int i = 0; i < responses.length; i++) {
            ...
            long expectedTimestampMillis =
                    (long) (new Double(responses[i].getTimestamp()) * 1000d);
            ...
            assertEquals(
                    ...
                    order.getOriginalAmount() - (order.getRemainingAmount()), 1e-8);
